    ...
    try {
        connection = db.getConnection((err, connection)=>{
            ...
            connection.beginTransaction((err)=>{
                ...
                const {reNo, content, rating, prNo } = req.body
                const reviewQuery = "UPDATE reviewtbl SET content=?, rating=? WHERE reNo=?"
                connection.query(reviewQuery, [content, rating, reNo], (err, result)=>{
                    if (err) {
                        return connection.rollback(() => {
                            connection.release();
                            res.status(500).send(err);
                    ...
                    if (result.affectedRows==1) {
                        const updateProductQuery = `
                                                    UPDATE producttbl 
                                                    SET averageRating = (SELECT AVG(rating) FROM reviewtbl WHERE prNo = ?) 
                                                    WHERE prNo = ?
                                                `;
                        connection.query(updateProductQuery, [prNo, prNo], (err, result)=>{
                            if (err) {
